...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	// 게시물 1개 조회작업
	ArchiveDTO archive = dao.selectCommBoard(num);
	
	// 로그인한 아이디와 글작성자가 다르다면 조회수1증가
	// 같다면 조회수 증가시키지 않음
...
<script type="text/javascript">
	...
	function forwardReply(){
		...
		location.href = "community_content_reply_writePro.jsp?boardType=archive&num=<%=archive.getNum()%>&name=<%=sId%>&content=" + content;
	...
	function warning(){
		...
		window.open("../admin/reportThisPost2archive.jsp?boardType=archive&num=<%=archive.getNum()%>&pId=<%=archive.getName()%>&pSub=<%=archive.getSubject()%>&sId=<%=sId%>", "신고하기", "width=390,height=500");
	...
	<div id="wrap">
		...
		<article>
			...
			<table id="notice" >
				...
				// 파라미터 : 글번호(num),게시판 종류(boardType) 	리턴타입 : List<ReplyDTO>(replyList)
			...
			<!--  댓글 작성자와 세션 아이디가 같으면 삭제버튼 표시 -->
				<%if(sId != null && sId.equals(reply.getName())) {%>
					<td>
					<a href="community_content_reply_deletePro.jsp?num=<%=reply.getNum()%>&refNum=<%=num%>">
					<img src="../images/delete_icon.png" width="20px" height="20px"></a>
					</td>
					<% } else {%>
					<td></td>
					<%} %>
					<td colspan="3" class="left"><%=reply.getContent() %></td>
					<td><%=reply.getName() %></td>
...
<%-- ********************************여기문제*************************************--%>
			...
			<%}%>
				<input type="button" value="글목록" class="btn" onclick="location.href='community.jsp'">
