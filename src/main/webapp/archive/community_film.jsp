	...
	<div id="wrap">
		...
		<article>
			<div id="board_head"><h1>Community</h1></div>
				<div class = "btn4type">
					<input type="button" id="a" value="ALL" onclick="location.href='community.jsp'">
					<input type="button" id="f" value="FILM" name="film" onclick="location.href='community_film.jsp?type=film'">
					<input type="button" id="d" value="DIGITAL" name="digital" onclick="location.href='community_digital.jsp?type=digital'">
				</div>
				<div id="photolist">
				<% 
				...
				ArrayList<ArchiveDTO> commBoardList = dao.selectCommBoardList(startRow, listLimit, type);
				...
				for(ArchiveDTO archive : commBoardList){
					...
					// ArchiveDAO 객체의 replyCount(refNum,boardType)	
			...
			int archiveListCount = dao.selectCommListCount(type);
			...
			<%} else  {%>
				<a href="community.jsp?pageNum=<%=pageNum - 1%>">Prev</a>
			<%} %>	
				...
				<%} else { %>
					<a href="community.jsp?pageNum=<%=pageNum + 1%>">Next</a>
				<%} %>
			</div>
		</article>
