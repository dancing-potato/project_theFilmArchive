...
<title>ID중복체크</title>
<script>
// 이게 익명함수 -> 한번쓰고 버리는, 이름몰라도 되는 애들.
// 윈도우온로드+익명함수 => 하단 getCheckId랑 같음
...
// 	}
	...
	function getCheckId(){
		// 만약, 파라미터로 전달받은 id가 null이 아니면 아이디 입력란에 해당 아이디 표시
		<%if(id != null && id.length()>=4 && id.length()<=12 && id != "") { %>
			...
			<%} else {%>
				...
				document.getElementById("checkIdResult").style.color = "BLUE";
			<%}%>
			
		<%} else {%>
		document.getElementById("checkIdResult").innerHTML = "아이디는 4~12자로 입력해주세요.";
		<%}%> 
	}
	// 사용가능한 아이디일때 아이디 사용 버튼 클릭 시 호출되는 메서드
	function useId(){
